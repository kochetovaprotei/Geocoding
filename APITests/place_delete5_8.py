...
class TestNewLocation:
    ...
    def test_create_new_location(self):
        ...
        check_info_delete = check_delete.get('status')  # получаем статус-код из тела запроса (в каком-то поле, значении)
        print('Status-code: ' + check_info_delete)
        ...
        print('Status updated location is right!')
        """Проверка адреса"""
        check_msg_address = result_get.json()
        check__msg_address_info = check_msg_address.get('msg')
        print('Message from response: ' + check__msg_address_info)
        assert check__msg_address_info == "Get operation failed, looks like place_id  doesn't exists"
        print('Remove location is successfull!')
